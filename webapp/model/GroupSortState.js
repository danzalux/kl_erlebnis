sap.ui.define([
	...
	return BaseObject.extend("kl-ee.KL-EE.model.GroupSortState", {
		 ...
		 * @alias kl-ee.KL-EE.model.GroupSortState
		 */
		...
		/**
		 * Sorts by Name, or by Price/Amount
		 *
		...
		/**
		 * Groups by Price/Amount, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Price/Amount") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "Price/Amount");

				aSorters.push(
					new Sorter("Price/Amount", false,
